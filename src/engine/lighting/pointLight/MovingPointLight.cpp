...
const double MovingPointLight::MAX_TRAVEL_DISTANCE{ 20.0f };

void MovingPointLight::update(const float moveBy, const float y) {
	const auto newPosition = position + direction * moveBy;
...
MovingPointLight MovingPointLight::createRandomPointLight(glm::vec3 position, glm::vec3 direction)
{
    ...
    return MovingPointLight(position, direction, color, 0.0f, 0.3f, 0.007f, 0.00008f);
}
