...
void VBO::createVBO(size_t reserveBytes) {
...
void VBO::bindVBO(GLenum bufferType) {
	if (!m_isBufferInitalized) {
...
void VBO::addRawData(const void* data, size_t dataSize, int repeat) {
...
void VBO::uploadDataToGPU(GLenum usageHint) {
    if (!m_isBufferInitalized)
    {
